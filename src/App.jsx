...
const App = ({ userSysInfo, updateSysInfo }) => {
  useEffect(() => {
    if (userSysInfo.navigation.navigation === 'auth' || userSysInfo.user.token.length === 0) {
      ...
      // if (userSysInfo.navigation.navigation === 'auth' && userSysInfo.user.token.length > 0)
      // if (userSysInfo.user.token.length !== 0) {
      //   updateSysInfo({ navigation: { navigation: 'userinfo' } })
      // }
      return history.push(`/${userSysInfo.navigation.navigation}`);
    }
